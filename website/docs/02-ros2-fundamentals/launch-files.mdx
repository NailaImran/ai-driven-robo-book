...
**Features**:
- ✅ 5 nodes launched together
- ✅ Parameters loaded from YAML files
- ✅ Conditional node launching (`enable_camera`)
- ✅ Topic remapping (`/image_raw` → `/camera/image_raw`)
...
my_robot/
├── launch/
│   ├── robot_bringup.launch.py        # Main entry point
...
✅ **Launch files automate startup**: One command for entire system
✅ **Python launch files**: Full programming language, not XML
✅ **Arguments for flexibility**: Override parameters at runtime
✅ **Include for composition**: Build large systems from smaller launch files
✅ **Conditions for environments**: Dev, sim, production configs
✅ **Namespaces for multi-robot**: Run multiple instances of same nodes
...
👉 **[Module 2: Robot Simulation with Gazebo](/docs/03-gazebo-simulation/)** — Build realistic robot simulations
