...
├── resource/                # Resource index marker
├── test/                    # Unit tests
...
├── include/my_cpp_controller/  # Header files
└── src/                     # Source files
```
...
├── srv/                  # Custom services
│   └── CustomSrv.srv
├── my_robot_package/     # Python source
│   ├── __init__.py
│   ├── node1.py
│   └── utils.py
├── test/                 # Tests
...
✅ **Packages organize code**: Nodes, messages, launch files, config
✅ **Python packages use `ament_python`**: Fast iteration, research
✅ **C++ packages use `ament_cmake`**: Performance, production
✅ **Custom messages**: Define in separate package, share across nodes
✅ **Colcon builds workspaces**: `colcon build`, `source install/setup.bash`
✅ **Parameters for configurability**: YAML files, command-line args
...
👉 **[Launch Files & Parameters](/docs/02-ros2-fundamentals/launch-files)** — Automate node startup
